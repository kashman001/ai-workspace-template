...
## 2026-09-18 — Phase 5 supervisor on the record: three verdicts, what still mirrors (agent, plans/phase-5.md)

**What:** `session-loop.sh` (1002→381 lines) checks the chain budget, stages through the launcher if nothing is staged, exports `TF_SESSION_PROJECT`/`TF_SESSION_SEQ`, records the child in `chain`, runs it, and reaches `staged`, `quit_stop`, `quit_plain` or `cap`, else broken (`rc_nonzero`, `logout`, `staged_invalid leg=<which>`, `no_own_measurement`, `record_unreadable`, `schema_mismatch`, `stall`); start refusals `record_unreadable`, `schema_mismatch`, `chain_closed`, `supervisor_live`, `relaunch_off`. A spent stage (successor already has an owner) is `staged_invalid leg=spent`, exit 4. `no_own_measurement` reads the record's `registered_at`, not registry mtimes; `--min-lifetime` stays as `leg=lifetime`. The stall guard watches the three markdown files plus `handoff-archive.md`. The watchdog and its pages stay, reading `session.pid`/`session.artifact`; its stop flag is a `mktemp` path. Verdict/refusal lines are stderr + log; tests pin exit code and code only. `--reset-cap` stays a standalone action (exit 0, no start). The `--clear` prompt is not the supervisor's (a supervised chain never runs `--clear`). Deleted: sentinel, flush-hash check, budget/alarm-stop/chain-closed state files, the launcher's `.session-seq` and emit `.json` sidecar. **Still written:** the `.session-loop` marker beside `chain.supervisor` (measurer `supervised` and the launcher's bootstrap exemption read it) and `.next-command` + `.session-seq.bump.json` (hook-lib `budget_hook_should_exit` reads them for the turn-end self-kill) — the agent may not edit the measurer or hooks. **Why:** the ticket's code list is closed; a re-staged spent number would bump past a running session; a committed knob that silently does nothing is worse than keeping it; an unattended chain must still page; dropping either mirror would fail every supervised rollover (`no_supervisor`) or keep every staged session alive. **Rejected:** a new `stage_spent` code; silently discarding a spent stage; dropping `--min-lifetime`; counting the record or counter files as progress; deleting the watchdog; pinning halt prose; `--reset-cap` also starting; the supervisor injecting `launch.pending.prompt`; dropping the marker or the hook mirror now. **Handoff (phase 7/8):** `supervised` reads `chain.supervisor`; hook-lib reads `staged.by`; then delete the marker, `.next-command`, the bump record, the launcher's legacy `.chain-closed` read, and the stale `.gitignore` lines. **Parent (session 14):** `test-session-lib.sh` S10a lock flake on the merge run, green twice on rerun, third lock-race sighting — phase 7 fixes the lock loop (retry when the lock vanishes between `mkdir` and the `-d` check) instead of another rerun.

## 2026-09-21 — Phase 7 probes as self-checking scripts, stub twins, lock fix, acceptance in a clone (agent, plans/phase-7.md)

**What:** the V1 (attended rollover), V3 (supervised chain) and V10 (fleet + parent rollover) probes are driver scripts under `evaluation/probes/` sharing one assertion library; `scripts/tests/test-probe-twins.sh` (37 asserts) runs the same scripts under a stub `claude` on PATH that honours the hooks contract (real `register` at SessionStart, the real Stop-hook dispatcher and its self-kill, `release` at exit), so CI and the acceptance run are literally the same assertions. V3 passes on the log's `verdict=` codes, `chain.used`, `staged`/`chain.closed` and the supervisor's exit code — the prose `successor: NOT STAGED` is not read. V10 asserts the child-lock gate is gone (two dispatch generations open, the parent's rollover succeeds). Two suite cases: H1 spent stage (the staged command run by hand, then the supervisor → `staged_invalid leg=spent`); H2 refused hand `--emit` (`not_owner` for the session that already staged, `owner_live` for a stranger). The `session-lib.sh` lock loop retries only a lost race (`mkdir` failed, no lock dir, record dir writable, nothing non-directory at the lock path), bounded by the same wait budget; lib suite 64 asserts, five runs green. The Claude Code acceptance ran in a `git clone` of the branch under the scratchpad: V1 with `claude -p` appended to the launcher's own `run:` command (18/18), V3 with the supervisor under `expect` and `--max-sessions 2` (15/15: `staged` seq 1, `staged` seq 2, `cap`). **Mirror removal left to phase 8:** `.session-loop` has a third reader outside the allowed edits (the launcher's `invoked_by_supervisor`), and `.next-command` is also `--emit`'s output contract; the exact readers and pins are in the plan's Concerns 1. **Why:** a twin with its own copy of the checks drifts from the acceptance; a stub that writes the `session` block directly never exercises the measurer, dispatcher or self-kill; every script resolves its root via `git rev-parse --git-common-dir`, so from a worktree the scripts would have driven the main checkout's `work/` (live supervisor there); an unconditional lock retry would spin an absent directory to a misleading `lock_timeout`; a half mirror removal would break every bootstrap. **Rejected:** a separate twin suite; the phase-5 stub; expect-driving the V1 TUI; editing the staged command; an env root override in the scripts; `work/s4-scratch/` inside the worktree; a new supervisor code for the hand-run stage; taking the mirror removal now. **Handoff (phase 8):** the mirror removal with its listed readers/pins; a docs line that a supervised session's tool shells inherit `TF_SESSION_LOOP=1` (a hand `--emit` from a subagent is refused `no_supervisor`; the probe lib unsets the four `TF_SESSION_*` vars); a fresh-clone TUI child needs the folder-trust dialog answered once (`claude -p` never records it).

## 2026-09-21 — Phase 8 mirrors gone, `/clear` seed via `register`, ADR set, doc-consistency test (agent, plans/phase-8.md)

**What:** the record is the only session state. `context-budget.sh supervised` reads `chain.supervisor` with the supervisor's liveness rule (pid alive and `pid_start` equal; dead or restarted pid = ambiguous exit 2, the three exits kept). The launcher's bootstrap exemption tests `chain.supervisor.pid == $PPID` and alive. The turn-end self-kill reads `staged != null` and `staged.by == my id` under `TF_SESSION_LOOP=1`; the `.rollover-complete` shim goes with it. Deleted: `.session-loop`, `.next-command`, `.session-seq.bump.json` writes, the legacy `.chain-closed` read, every pin. `--emit` takes no path and prints the staged command as `cmd: <line>` (same line `--dry-run` prints); `--loop-reason` lands in `launch.reason`. The `/clear` injector is `register` itself: on a `via=pending` bind it prints the pending prompt on stdout (Claude Code adds SessionStart stdout to context) and nulls `launch.pending` in the same compare-and-set write; the stage4 clear-seed settings entry is removed. Doc-consistency test: backticked tokens under the doc's `## Verbs and reason codes` versus the dispatch `case` arms and every `refuse|broken|verdict <code>` / `reason=` literal in the scripts, both directions empty (12 verbs, 43 codes). ADRs: new 0010 (one record, three writers; supersedes 0007/0008's mechanism), 0011 (mechanical gates and reason codes), 0012 (runtime contract, adapter table, fleet isolation); 0004/0005/0006/0009 amended by dated notes. The "three Open change-log entries" never existed (stage-1 re-evaluation F8); the two `Status:` blocks were rewritten instead. `.gitignore` names each written file explicitly. Vendor configs keep the shim paths. **Why:** a second copy of the command with no reader is exactly the mirror this phase removes; a hook that must never block a turn gains nothing from a second liveness read; the smallest injector is the hook that already runs; a data file listing codes would be a third copy to drift; repointing codex's config re-prompts its trust hash on every checkout. **Rejected:** optional `--emit <path>` copy; two-exit `supervised`; full pid+start rule in the exemption; a second SessionStart entry with `additionalContext`; a machine-readable code list; inventing three entries to close; one omnibus ADR; `session-state.json*` glob; repointing vendor configs now. **Concerns (parent):** the `/clear` injector is stub-verified only — one attended `--clear` at cutover confirms it, fallback is an `additionalContext` envelope from the same `register`; `attach-session.sh` and `statusline-context-budget.sh` still read `.active-session` (follow-up onto the record's `session` block); launcher and supervisor must land together at cutover (they do, on `stage4`).
