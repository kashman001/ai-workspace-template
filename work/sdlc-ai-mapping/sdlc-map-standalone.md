...
The lifecycle is presented three times on purpose. The **numbered list**
is the narrative view — the order a single change experiences the stages,
easiest to reason and talk about. The **graph** is the structural view —
what the system of many concurrent changes actually looks like, feedback
edges included. The **artifacts table** is the output view — the same
stages seen through what each one produces, which is where the AI overlay
gets concrete. None replaces the others.

### View 1 — the phases (one change's journey)
...
### View 3 — the artifacts (what each stage produces)
...
Evidence tiers in the AI column are the four defined in the Legend
above. Where a human is irreplaceable, the row says so.
