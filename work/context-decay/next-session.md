...
same day. This project is **dormant** — only reopen it when a gate below
clears or the ledger warrants a fresh analysis pass.
...
4. **Next ledger analysis** — after ~20 entries or the first
   `method=estimate` rows (currently 11 entries, all claude/exact).
