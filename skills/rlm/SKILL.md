...
> engine, swap the `_claude_exe()`/`llm_query` subprocess call in `rlm_repl.py`.
> Knobs are env-overridable: `RLM_SUB_MODEL` (default `haiku`), `RLM_MAX_WORKERS`
> (8), `RLM_MAX_DEPTH` (1), `RLM_ROOT_MODEL` (`sonnet`).
> Source: github.com/brainqub3/claude_code_rlm (MIT, John Adeojo) — see `LICENSE`.
