#!/usr/bin/env bash
# File: scripts/tests/test-fleet-dispatch-records.sh
# Purpose: Regression tests for the dispatch-record subcommands in
#          fleet.sh (dispatch-open / dispatch-close / dispatch-list)
#          — R4 of the subagent-rollover research (parent-persisted dispatch
#          records + generation fencing); moved out of context-budget.sh in
#          Stage 4 phase 2, which now refuses them (L10). Self-contained:
...
out=$("$FLEET" dispatch-open --project testproj --task task-1 --report "$REPORT" \
...
err=$("$FLEET" dispatch-open --task t --report r 2>&1 >/dev/null); rc=$?
assert_eq "L2a: no --project dies 3" "$rc" "3"
err=$("$FLEET" dispatch-open --project testproj --report r 2>&1 >/dev/null); rc=$?
assert_eq "L2b: no --task dies 3" "$rc" "3"
assert_contains "L2c: error names --task" "$err" "--task"
err=$("$FLEET" dispatch-open --project testproj --task t 2>&1 >/dev/null); rc=$?
assert_eq "L2d: no --report dies 3" "$rc" "3"
err=$("$FLEET" dispatch-open --project testproj --task t --report r --gen 2 \
  2>&1 >/dev/null); rc=$?
assert_eq "L2e: --gen dies 3" "$rc" "3"
assert_contains "L2f: error says gen is computed" "$err" "computed"
err=$("$FLEET" dispatch-open --project nosuch --task t --report r 2>&1 >/dev/null); rc=$?
assert_eq "L2g: unknown work dir dies 3" "$rc" "3"

echo "L3: generation fencing — open refuses while a generation is open"
err=$("$FLEET" dispatch-open --project testproj --task task-1 --report "$REPORT" \
  2>&1 >/dev/null); rc=$?
...
err=$("$FLEET" dispatch-close --project testproj --task task-1 --status DONE \
  2>&1 >/dev/null); rc=$?
...
err=$("$FLEET" dispatch-close --project testproj --task task-1 --status NOPE \
  2>&1 >/dev/null); rc=$?
assert_eq "L5a: invalid status dies 3" "$rc" "3"
err=$("$FLEET" dispatch-close --project testproj --task task-1 2>&1 >/dev/null); rc=$?
assert_eq "L5b: missing status dies 3" "$rc" "3"
"$FLEET" dispatch-close --project testproj --task task-1 --status KILLED \
...
err=$("$FLEET" dispatch-close --project testproj --task nosuch --status DONE \
  2>&1 >/dev/null); rc=$?
assert_eq "L5f: close on unknown task dies 3" "$rc" "3"

echo "L6: dispatch-list — per-task lines, exit 1 iff any generation open"
"$FLEET" dispatch-open --project testproj --task task-2 --report \
  work/testproj/task-2-report.md >/dev/null 2>&1
out=$("$FLEET" dispatch-list --project testproj 2>/dev/null); rc=$?
...
"$FLEET" dispatch-close --project testproj --task task-2 --status DONE >/dev/null 2>&1
out=$("$FLEET" dispatch-list --project testproj 2>/dev/null); rc=$?
assert_eq "L6g: exit 0 when all closed" "$rc" "0"
out=$("$FLEET" dispatch-list --project emptyproj 2>&1); rc=$?
...
out=$("$FLEET" dispatch-contract --report "$REPORT" 2>/dev/null)
...
out=$("$FLEET" dispatch-open --project testproj --task task-3 --report \
...
echo "L10: the measurer refuses the moved verbs and names the new script"
err=$("$CB" dispatch-open --project testproj --task task-9 --report r 2>&1 >/dev/null); rc=$?
assert_eq "L10a: dispatch-open through the measurer exits 3" "$rc" "3"
assert_contains "L10b: message names scripts/fleet.sh dispatch-open" "$err" "scripts/fleet.sh dispatch-open"
[ -f "work/testproj/.agent-dispatch/task-9.json" ] \
  && bad "L10c: measurer wrote no record" || ok "L10c: measurer wrote no record"
err=$("$CB" dispatch-list --project testproj 2>&1 >/dev/null); rc=$?
assert_eq "L10d: dispatch-list through the measurer exits 3" "$rc" "3"
