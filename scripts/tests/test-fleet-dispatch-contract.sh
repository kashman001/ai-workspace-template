#!/usr/bin/env bash
# File: scripts/tests/test-fleet-dispatch-contract.sh
# Purpose: Regression tests for the `dispatch-contract` subcommand in
#          fleet.sh — R2/R3 of the subagent-rollover research
#          (work/automatic-session-rollover, slice 3); moved out of
#          context-budget.sh in Stage 4 phase 2. Self-contained:
#          builds a throwaway workspace + fake $HOME in mktemp -d.
set -u
SRC_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
TMP="$(mktemp -d)"; trap 'rm -rf "$TMP"' EXIT
mkdir -p "$TMP/scripts" "$TMP/work/testproj" "$TMP/home"
cp "$SRC_ROOT/scripts/fleet.sh" "$TMP/scripts/"
...
out=$("$FLEET" dispatch-contract --report "$REPORT" 2>/dev/null); rc=$?
...
err=$("$FLEET" dispatch-contract 2>&1 >/dev/null); rc=$?
...
out=$("$FLEET" dispatch-contract --report "$REPORT" --brief "$BRIEF" 2>/dev/null)
...
out=$("$FLEET" dispatch-contract --report "$REPORT" --gen 2 2>/dev/null)
...
out=$("$FLEET" dispatch-contract --report "$REPORT" --brief "$BRIEF" --gen 2 2>/dev/null)
...
out=$("$FLEET" dispatch-contract --report "$REPORT" --runtime codex 2>/dev/null); rc=$?
