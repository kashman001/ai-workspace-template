...
# resolve_repo_path() — shared with check-repo-context.sh (see scripts/lib/repo-paths.sh).
# When no path is passed explicitly (including --refresh, whose hint is pathless), prefer
# the registry "Location" so a repo onboarded in place at an external/local path is found.
. "$ROOT/scripts/lib/repo-paths.sh"
